...
- **Canvas** — the dashboard rendered through the same `Responsive` + `useContainerWidth()` engine used at runtime, in **edit mode**: drag + resize via react-grid-layout, honoring `minW/minH` from `LayoutItem`. There is **one canonical (widest) layout**; the editor never exposes per-breakpoint editing. On drag/resize it captures the widest layout from RGL's `allLayouts` and writes it back as `layout`. Cell metrics come from `editorGridMetrics()` (canonical column count at every width, cell SIZE scaled to fit).
- **Widget chrome** — a widget shows nothing at rest. Hover (or keyboard focus, or selection) fades in a faint 1px ring plus the edit / duplicate / delete cluster at the top-right. The cluster keeps `pointer-events: auto` at every opacity, so a click landing mid-fade still hits the button instead of the full-body drag layer underneath.
- **Add (in-context)** — there are **no add buttons in the toolbar**. `InsertLines` overlays two sets of invisible lines and reveals the ONE nearest the pointer (within ~10px), as a hairline with a round `+` that opens a **Chart · Text · Input** menu:
  - **Horizontal (row) lines**, at every row boundary — `rowBoundaries()`: the top, each item's bottom edge, **minus** any boundary a taller widget straddles (an insert there overlaps the straddler, so the compactor would move the newcomer somewhere other than where the line promised). Choosing a kind calls `insertWidgetAtRow()`: the widget takes `x:0, y:row` with its kind's `DEFAULT_FOOTPRINT`, and everything at or below slides down.
  - **Vertical (column) lines**, in the column gaps *inside* one row band — `columnBoundaries()`: per band (the space between two consecutive row boundaries), each item's right edge, minus the canvas edges (`0`/`cols`) and any edge a wider band-mate straddles. A band whose widgets stop short of `cols` keeps that last edge as its **free right edge**. A column line is only as tall as its own band, and only answers to a pointer inside it. Choosing a kind calls `insertWidgetAtColumn()`, which puts the widget *beside* its neighbours at `{x: colX, y: rowY}` and makes room in three escalating steps: **shift** the row-mates right; else **squeeze** them proportionally into the columns that are left (never below their `minW`, and never below a 2-column floor when they declare none); else **push** them down and let RGL's compactor settle. On a free right edge nobody moves — the widget just fills what's left (`w = min(fp.w, cols - colX)`).

  Exactly one line is ever active — the nearest boundary across BOTH sets — so a row line and a column line never light up together. Hit-testing measures the pointer against the boundary offsets instead of laying transparent hit bands over the canvas: a band big enough to catch would overlap the widgets either side of it and swallow their drags and corner resizes. The layer is `pointer-events: none` except the active line's button, and hides entirely while an RGL drag/resize is in flight, so it never competes for a drag. An **empty board** shows `.cv-editor-empty` instead: a title, one line of hint, and three large tiles for the same three kinds. `onCreateChart` still intercepts the CHART kind (the host's own creation wizard inserts through `spec` + `openWidgetId`).
- **Variables (docked)** — the toolbar's Variables toggle opens `VariablesDock`, a 22rem right-hand dock **beside** the canvas (own scroll, `border-left`); the canvas stays mounted and live, so you can see the input widgets you are declaring variables for. Each row carries name / type / label / default / array plus a **"Used by 2 inputs · 5 queries"** line. A rename goes through `renameVariable()` (declaration + every `{var:name}` token + every `control.variable`), so nothing is orphaned; a delete of something in use asks first ("Remove (in use by N)") and then `removeVariable()` unbinds — input widgets get `control.variable = ""` and `{var}` tokens are replaced by the variable's `default` (removed when it has none). Both live in the pure `dashboard/variableUsage.ts`.
- **Select-to-edit** — clicking a widget selects it (ring); its pencil opens a **full-screen editor** over the whole surface, hosting the chart editor (chart widgets), the TipTap editor (text), or the control config (input). The canvas is unmounted while it is open (it is fully occluded, and background `CubeChart`s would re-render on every keystroke), and the variables dock stays out of the way.
- **Edit metadata** — every change fires `onChange(nextSpec, meta: EditMeta)`. `meta.kind` is one of `layout | name | variables | widget | text | add | remove | duplicate`, `meta.label` is a short imperative phrase for a tooltip (`edit "Fuel by week"`), and `meta.coalesceKey` marks commits the host should fold into ONE undo step (`"name"`, `"variables"`, `widget:<id>:<session>`). Closing a widget editor ends its session, so re-opening the same widget later is its own undo step. The host owns the stack and feeds `undoLabel`/`redoLabel` back to the toolbar, which renders "Undo move widget" as the button's tooltip + accessible name.
...
- **Mobile-WebView:** the same URL loads in a device WebView (e.g. an Expo/React-Native `WebView` pointed at the LAN address, or the device browser). Because the runtime layout engine is **container-width reflow** (`useContainerWidth()` + RGL auto-derived narrow layouts — no breakpoints), the dashboard reflows correctly to the phone's WebView width with zero mobile-specific layout code. Widget editing is a **full-screen surface** (never a native sheet — the same component web and WebView), so editing works on the phone too (A3.2).
