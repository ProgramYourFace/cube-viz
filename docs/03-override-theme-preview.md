...
- **Add (in-context)** — there are **no add buttons in the toolbar**. `InsertLines` overlays an invisible line at every *row boundary* of the layout (`rowBoundaries()`: the top, each item's bottom edge, minus any boundary a taller widget straddles). Bring the pointer within ~10px and the line fades in with a round `+`; the `+` opens a **Chart · Text · Input** menu that inserts AT that row via `insertWidgetAtRow()` (everything at or below slides down). A chart opens straight into its editor; text/input are edited in place. The overlay is `pointer-events: none` except the active line's button, and hides entirely while an RGL drag/resize is in flight, so it never competes for a drag. An **empty board** shows `.cv-editor-empty` instead: a title, one line of hint, and three large tiles for the same three kinds. `onCreateChart` still intercepts the CHART kind (the host's own creation wizard inserts through `spec` + `openWidgetId`).
