  ...
  return (
    <Popover>
      <PopoverTrigger
        className={cn(
          "cv-filters-trigger",
          count > 0 && "cv-filters-trigger--active",
        )}
        title="Filters"
        aria-label="Filters"
      >
        <ListFilter className="cv-ec-icon--lg" />
        Filter
        {count > 0 ? (
          <span className="cv-filters-count">
            {count}
          </span>
        ) : null}
      </PopoverTrigger>
      <PopoverContent align="end" className="cv-filters-popover">
        <div className="cv-filters-header">
          <p className="cv-filters-title">Filters</p>
          <p className="cv-filters-desc">
...
function SegmentsControl({
  ...
  return (
    <div className="cv-filter-segments">
      <p className="cv-filter-segments-heading">Segments</p>
      <div className="cv-filter-segments-list">
        {segs.map((s) => (
          <button
            key={s.name}
            type="button"
            ...
            className={cn(
              "cv-segment-chip",
              active.has(s.name) && "cv-segment-chip--active",
            )}
          >
            {s.label}
          </button>
