...
import type { WellDef } from "@/editor/chart/builder/wells";
...
import { MapChartFamily } from "./map";
...
const MAP_WELLS: WellDef[] = [
  ...
  { id: "weight", label: "Weight", hint: "heatmap intensity", cardinality: "one", kinds: ["number"], optional: true },
  { id: "series", label: "Split by", hint: "color points by category", cardinality: "one", kinds: ["category"], optional: true },
  { id: "time", label: "Order by time", hint: "orders path vertices", cardinality: "one", kinds: ["time"], optional: true },
];

/**
 * The builtin family → descriptor table. The ORDER of the keys here is the picker's
 * `order` (assigned below), matching the original `FAMILY_ORDER`.
 */
const ORDER: ChartFamily[] = ["bar", "line", "area", "pie", "scatter", "kpi", "table", "combo", "map"];
const orderOf = (family: ChartFamily): number => ORDER.indexOf(family);

export const builtinFamilyDescriptors: Record<ChartFamily, ChartFamilyDescriptor> = {
  bar: {
    family: "bar",
    label: "Bar",
    icon: BarChart3,
    ...
    optionsSchema: familyOptionsSchema("bar"),
  ...
  line: {
    ...
    optionsSchema: familyOptionsSchema("line"),
  ...
  area: {
    ...
    optionsSchema: familyOptionsSchema("area"),
    defaults: DEFAULTS.area,
    wells: CARTESIAN_WELLS,
    zones: { left: ["y"], bottom: ["x", "color"] },
    dualAxisY: false,
    supportsMapping: true,
  ...
  pie: {
    ...
    optionsSchema: familyOptionsSchema("pie"),
    defaults: DEFAULTS.pie,
    wells: PIE_WELLS,
  ...
  scatter: {
    ...
    optionsSchema: familyOptionsSchema("scatter"),
    defaults: DEFAULTS.scatter,
    wells: SCATTER_WELLS,
  ...
  kpi: {
    ...
    order: orderOf("kpi"),
    component: KpiFamily,
    optionsSchema: familyOptionsSchema("kpi"),
    defaults: DEFAULTS.kpi,
    wells: KPI_WELLS,
  ...
  table: {
    ...
    optionsSchema: familyOptionsSchema("table"),
  ...
  combo: {
    ...
    optionsSchema: familyOptionsSchema("combo"),
    defaults: DEFAULTS.combo,
    wells: COMBO_WELLS,
  ...
  map: {
    ...
    optionsSchema: familyOptionsSchema("map"),
...
export function familyDescriptor(family: ChartFamily): ChartFamilyDescriptor {
  return builtinFamilyDescriptors[family];
}
